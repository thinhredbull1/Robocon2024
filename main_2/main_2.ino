...
void readEncoderM2(const char* message, bool pinstate) {
  int b = digitalReadFast(encb[M2]);
  if (b > 0) {
    encoder_count[M2]--;
  } else {
    encoder_count[M2]++;
...
void setup() {
  ...
  for (int k = 0; k < NMOTORS; k++) {
    ...
    pid[k].setParams(15.2, 128.5, 0, 255);  //39.2 34.6
  }
  // pid[k].setParams(7.6, 0, 0.01, 255);  //39.2 34.6
  PcInt::attachInterrupt(enca[M2], readEncoderM2, "", RISING);
  attachInterrupt(digitalPinToInterrupt(enca[M3]), readEncoderM3, RISING);
}
void loop() {
